...
/// - backslash escapes (`\*`, `\_`, `\[`, `\$`, ...)
/// - `$math$` inline and `$$math$$` display LaTeX (the opening `$` must be
///   followed, and the closing one preceded, by a non-space; a `$` followed
///   by a letter or digit never closes, so `$5 and $10` and
///   `($K) and ($M)` stay text)
///
...
pub fn parse(text: &str) -> Vec<Inline> {
    ...
    while i < chars.len() {
        ...
        if c == '$'
            && let Some((inline, end)) = try_math(&chars, i)
        {
...
fn try_math(chars: &[char], start: usize) -> Option<(Inline, usize)> {
    if peek(chars, start + 1) == Some('$') {
        ...
        match chars[j] {
            ...
            '$' => {
                let closes = !chars[j - 1].is_whitespace()
                    && !peek(chars, j + 1).is_some_and(|c| c.is_alphanumeric());
                if closes {
