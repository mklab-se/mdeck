...
pub fn parse(text: &str) -> Vec<Inline> {
    ...
    while i < chars.len() {
        ...
        if c == '*' || c == '_' {
            if let Some((inline, end)) = try_emphasis(&chars, i, c) {
                flush_text(&mut current_text, &mut result);
                result.push(inline);
                i = end;
                continue;
        ...
        // Strikethrough: ~~text~~
        if c == '~' && peek(&chars, i + 1) == Some('~') && run_len(&chars, i, '~') == 2 {
            if let Some(end) = find_closer(&chars, i + 2, '~', 2, true) {
                flush_text(&mut current_text, &mut result);
        ...
        if c == '!' && peek(&chars, i + 1) == Some('[') {
            if let Some((link, end)) = parse_link(&chars, i + 1) {
                flush_text(&mut current_text, &mut result);
                result.push(link);
                i = end;
                continue;
